...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=False, resolution_scales=[1.0], if_pretrain=False, pretrain_path=None,given_scale=1.0,given_center=[0,0,0]):
        """b
        ...
        if self.loaded_iter:
            if not if_pretrain:
                ...
                path, _ = os.path.split(self.model_path)
                new_path = pretrain_path #"test_hash_21_4_full_0_3.1_r10_add"
                # new_path = os.path.join(path, new_last_part)

                print("!!!using Pretrained Scaffold-GS:", new_path)
                # import pdb;pdb.set_trace()

                self.gaussians.load_ply_sparse_gaussian(os.path.join(new_path,
                                                            "point_cloud",
                                                            "iteration_" + str(self.loaded_iter),
                                                            "point_cloud.ply"))
                self.gaussians.load_mlp_checkpoints(os.path.join(new_path,
                                                            "point_cloud",
                                                            "iteration_" + str(self.loaded_iter),
                                                            "checkpoint.pth"))
    ...
    def save(self, iteration):
        ...
        self.gaussians.save_mlp_checkpoints(os.path.join(point_cloud_path))
